...
int main() {
  ...
  bot.on_message_create([&bot, &TOKENS](const dpp::message_create_t &event) {
    ...
    } else if ((int)event.msg.content.find("!em") == 0 && event.msg.content.size() > 2) {
    ...
    else if(event.msg.content.find("$$")==0 && event.msg.content.size()>3){
        std::string latex_msg=event.msg.content;
        latex_msg.erase(0,2);
        latex_msg.erase(latex_msg.length()-2);
        latex_msg.erase(std::remove(latex_msg.begin(), latex_msg.end(), ' '), latex_msg.end());
